...
const ColorPickerScreen = ({navigation}) => {
  ...
  // Get colors and presets from ColorManager
  const predefinedColors = ColorManager.getBasicColors();
  const colorPresets = ColorManager.getColorPresets();
  ...
  useEffect(() => {
    ...
    const handleConnectionChange = device => {
      setIsConnected(true);
      logger.info('Device connected in ColorPickerScreen', {device: device.name});
    };

    const handleDisconnection = () => {
      setIsConnected(false);
      logger.info('Device disconnected in ColorPickerScreen');
    };
  ...
  const handleColorSelect = async color => {
    try {
      ...
      const success = await ColorManager.applyColor(color);
  ...
  const handlePresetSelect = async preset => {
    try {
      ...
      const success = await ColorManager.applyPreset(preset.name);
